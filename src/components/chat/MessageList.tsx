...
function getToolLabel(toolName: string, args: Record<string, unknown>): string {
  if (toolName === "str_replace_editor") {
    ...
    switch (command) {
      case "create": return `Creating ${fileName || "file"}`;
      case "str_replace":
      case "insert": return `Editing ${fileName || "file"}`;
      case "view": return `Reading ${fileName || "file"}`;
      default: return "Editing file";
    }
  }
  if (toolName === "file_manager") {
    const command = args?.command as string;
    const path = (args?.source_path ?? args?.path) as string;
    const fileName = path ? path.split("/").pop() : "";
    switch (command) {
      case "rename": return `Renaming ${fileName || "file"}`;
      case "delete": return `Deleting ${fileName || "file"}`;
